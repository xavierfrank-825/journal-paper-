...
const handleUpload = async () => {
  ...
  try {
    const result = await uploadPaper(file); // use your existing API function
  ...
  const handlePreviewHTML = () => {
    ...
    const paperData = {
      title: file?.name.replace('.xml', '') || 'Uploaded Paper',
        ...
        <div className="upload-actions">
          <div className="action-buttons">
            <button
              ...
              disabled={uploading || !validateXMLContent(xmlContent)}
              className="btn btn-primary btn-lg"
              title={!validateXMLContent(xmlContent) ? 'Please fix XML validation errors first' : ''}
            >
              {uploading ? (
                <>
